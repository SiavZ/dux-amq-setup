...
- The `dux-amq` installer (`dux-amq/install.sh`),
  ...
  (`claude-amq`, `codex-amq`, `gemini-amq`, `jcode-amq`).
...
Out of scope: configured upstream provider binaries (including `ntl`) and
the `amq` binary; cloud-provider IAM; the host kernel.
...
|-----|---------------------------------------------------------------------------------------------------------|--------|------------------------|-----------------------------------------------------------------------|--------------|
| T1  | Malicious repo executes through permission, sandbox, or hook-trust bypasses                              | T,E    | Host shell, API tokens | Default-deny YOLO; `SessionSettings.yolo_permissions` is per-session and defaults to `false`, mapping to `CLAUDE_AMQ_YOLO=1` / `CODEX_AMQ_YOLO=1` for wrapper providers and native `opencode --auto` for OpenCode. A missing or corrupt `agent_sessions.session_settings` blob loads `SessionSettings::default()` (asymmetric default) so a tampered DB cannot escalate a session into YOLO. Codex hook trust review is preserved independently unless the operator explicitly sets `CODEX_AMQ_BYPASS_HOOK_TRUST=1`. Wrappers fail closed below reviewed provider-version floors or when the version cannot be parsed. | 01, hardened in audit03 phases 01/B/P1 |
...
| T19 | A selected third-party NTL provider sends prompts off-host or executes agent actions with the operator's Unix permissions | I,T,E | Workspace, prompts, host shell | NTL is an explicit config-only provider and is never installed or launched automatically. Dux invokes the official `ntl` executable without an adapter or private API access; one-shot use is forced to `--chat --no-color`, session resume is disabled, and the operator retains NTL's own approval and authentication controls. | NTL provider |
| T20 | jcode provider self-updates mid-session, spawns headless sub-agent swarms, or reaches fleet messaging through inherited MCP servers | T,I,E | Workspace, prompts, host shell, AMQ identity | jcode is a config-only provider entry; Dux never installs or launches it silently. `jcode-amq` fails closed below a reviewed provider-version floor, probes the version with `--no-update`, and force-injects `--no-update` into every wrapped launch (targeted resume repeats it in `resume_by_id_args`) so the pane's binary is pinned for the PTY's lifetime. Non-interactive subcommand invocations (`run`, `usage`, ...) bypass identity and wake claims entirely, so oneshot work cannot register or hold an AMQ handle. Native (non-PTY) wake delivery via claude-peers / the jcode daemon socket is opt-in (`DUX_AMQ_NATIVE_DELIVERY=1`), checks readiness before draining, falls back to the file queue when the daemon is unreachable, and logs every attempt; the debug socket it needs is a same-UID surface accepted under T2. The shared `serve` daemon, sub-agent swarms, and any inherited user-scope MCP servers run with the operator's Unix permissions and identity; their binaries, backends, and data handling remain upstream responsibilities. | jcode provider + jcode-amq |
