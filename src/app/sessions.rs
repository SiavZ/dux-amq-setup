...
impl App {
    ...
    pub(crate) fn apply_change_agent_provider(&mut self) -> Result<()> {
        ...
        self.session_store
            .upsert_session(&self.git.sessions[session_index])?;
    ...
    pub(crate) fn open_selected_worktree_in_default_editor(&mut self) -> Result<()> {
            ...
            && !editor::matches_configured_editor(&selected_editor, &configured_default)
        {
            self.set_info(format!(
                ...
                selected_editor.label, selected_editor.command, session.worktree_path, configured_default
            ));
        }
...
mod tests {
    ...
    fn test_app_with_sessions(sessions: Vec<AgentSession>, projects: Vec<Project>) -> App {
        ...
        let runtime = RuntimeState {
            ...
            watch_pending_enters: std::collections::HashMap::new(),
            ...
            orchestrator_project_last_checkpoint: std::collections::HashMap::new(),
    ...
    fn scrollback_auto_detach_picks_oldest_session() {
        ...
        let old_session = app.git.sessions.iter().find(|s| s.id == "old").unwrap();
