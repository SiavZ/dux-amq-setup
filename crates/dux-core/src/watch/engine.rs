...
/// mitigation described in the threat model in `watch/mod.rs`.
...
/// to run synchronously from the engine's per-tick poll.
...
mod tests {
    ...
    fn regex_size_limit_caps_oversized_patterns() {
        ...
        // catastrophic-regex mitigation from the `watch/mod.rs` threat model.
