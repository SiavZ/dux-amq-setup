//! Filesystem layer of the AMQ inject-queue drainer.
//!
//! `dux-amq-inject-bridge` (see `dux-amq/scripts/`) writes verified wake
//! bodies to `~/.local/share/dux-amq/inject-queue/<receiver>/<ts>.msg`
//! whenever it detects it's running inside a dux-spawned process tree (via
//! the `DUX_PANE` launch env). This module owns:
//!
...
//!    (`RENAME_NOREPLACE`, so no body is ever clobbered).
//! 4. Startup recovery (reclaim/expire) and the `notify` watcher.
//!
//! The "type body into the agent's PTY when idle" state machine lives in
//! `crate::engine::amq` (`Engine::tick_amq`).
...
use crate::amq::config::AmqInjectConfig;
...
pub struct QueuedMessage {
    ...
    /// enforces a minimum time gap between the two writes. Claude and
    /// Codex use explicit bracketed paste for phase 1 and still reuse
    /// the same phase-2 submit state.
...
/// Start a recursive `notify` watcher on `queue_dir` that calls `make_event`
/// and posts the result on `event_tx` for every create/modify under it.
/// Returns the handle; dropping it stops the watcher. The polling fallback
/// for filesystems where `notify` is lossy is the engine's loop worker
/// (`Engine::start_amq`), which honours the engine's shutdown flag.
pub fn spawn_inject_watcher<E>(
    queue_dir: &Path,
    event_tx: Sender<E>,
    make_event: impl Fn() -> E + Send + 'static,
    ...
    E: Send + 'static,
{
    ...
    let mut watcher = RecommendedWatcher::new(
        move |res: Result<notify::Event, notify::Error>| {
            ...
            // Creates and writes only: removes are the drainer's own unlinks.
