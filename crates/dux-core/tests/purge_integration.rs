//! Integration tests for `dux_core::purge`, the GDPR hard purge (fork
//! tests/purge_integration.rs, audit02 Phase 10 / P0-J).
//!
...
use dux_core::purge_encoding;
use dux_core::storage::{SessionStore, load_or_create_store_id, load_store_id};
...
impl PurgeHarness {
    fn new() -> Self {
        ...
        let session = AgentSession {
            ...
            deleted_at: None,
            slot_tab_id: "sid-target-slot".to_string(),
            ...
            workspace: AgentWorkspace::Managed(ManagedWorkspace {
                ...
                initial_branch: "audit02-x".to_string(),
    ...
    fn worktree(&self) -> PathBuf {
        PathBuf::from(self.session.directory())
    }
}

fn managed(session: &mut AgentSession) -> &mut ManagedWorkspace {
    session
        .workspace
        .as_managed_mut()
        .expect("harness sessions are managed")
}
...
fn assert_failure_retains_row_and_retry_succeeds(
    ...
    let failed_plan = dux_core::purge::PurgePlan {
        session_id: h.session.id.clone(),
        branch: h.session.branch_name().unwrap().to_string(),
...
fn purge_executes_in_documented_order() {
    ...
    // AmqInbox, StartupLogs, LogScopedRedact, SqliteRow.
    let kinds: Vec<&str> = plan
        ...
        .map(|i| match i {
            ...
            PurgeItem::StartupLogs(_) => "startup-logs",
    ...
    assert_eq!(
        ...
        "purge order must match the contract: worktree, providers, amq, startup logs, log, sqlite"
    );
...
fn purge_target_resolves_by_immutable_handle_and_rejects_ambiguous_branch() {
    ...
    managed(&mut sibling).worktree_path = h
        .paths
        .worktrees_root
        .join("sibling")
        .to_string_lossy()
...
fn purge_rejects_root_parent_and_symlink_worktree_targets() {
    ...
    for unsafe_path in [
        ...
        managed(&mut session).worktree_path = unsafe_path.to_string_lossy().into_owned();
        assert!(
    ...
    let mut session = h.session.clone();
    managed(&mut session).worktree_path = escape.to_string_lossy().into_owned();
...
fn purge_amq_target_uses_handle_even_when_branch_is_pathlike() {
    ...
    for branch in ["/tmp/audit03-outside", "../../audit03-outside", "."] {
        let mut session = h.session.clone();
        managed(&mut session).branch_name = branch.to_string();
...
fn purge_allows_missing_descendants_and_reports_validated_paths() {
    ...
    managed(&mut session).worktree_path = missing_worktree.to_string_lossy().into_owned();
    managed(&mut session).branch_name = "missing/session".to_string();
...
fn purge_all_retains_rows_whose_full_inventory_cannot_be_planned() {
    ...
    managed(&mut relative_worktree).branch_name = "relative-row".to_string();
    managed(&mut relative_worktree).worktree_path = "relative/worktree".to_string();
    h.storage
    ...
    managed(&mut absolute_branch).branch_name = "/tmp/bad\u{1b}]0;branch".to_string();
    managed(&mut absolute_branch).worktree_path = h
        .paths
        .worktrees_root
