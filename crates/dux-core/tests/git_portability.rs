//! audit02 Phase 21 (P2-11): git wrapper portability tests, ported from the
//! fork's `tests/git_portability.rs` (11d9274f).
//!
//! Verifies that `crates/dux-core/src/git.rs` invokes git with `&Path`/`&OsStr` arguments
...
fn create_worktree_existing_branch_handles_non_utf8_parent() {
    ...
    // Create a worktree under a *separate* non-UTF-8 worktrees root, which
    // doubles the surface area that must round-trip cleanly.
    let worktrees_root = tmp.path().join(OsString::from_vec(vec![0xff, b'w', b't']));
    let result = dux_core::git::create_worktree_existing_branch(
        &project_root,
        &worktrees_root,
        "demo",
        "feature",
