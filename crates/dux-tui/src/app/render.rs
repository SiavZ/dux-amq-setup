...
impl App {
    ...
    fn tab_provider_label(&self, session: &AgentSession, tab_id: &str) -> String {
        self.engine
            .tab_running_provider(session, TabIdRef::new(tab_id))
            .as_str()
            .to_string()
    }
    ...
    fn agent_terminal_context(&self) -> AgentTerminalContext {
        ...
        let session_active = match active_surface {
            SessionSurface::Agent => focused_tab
                .as_ref()
                .is_some_and(|id| self.engine.providers.contains_key(TabIdRef::new(id))),
    ...
    fn render_change_theme_prompt(&mut self, frame: &mut Frame) {
        let PromptState::ChangeTheme(prompt) = &self.prompt else {
            return;
        };
        self.render_dim_overlay(frame);
            ...
            visible
                ...
                .map(|(i, entry)| {
                    ...
                    ListItem::new(Line::from(vec![
                        ...
                        Span::styled(entry.label.clone(), Style::default().fg(self.theme.text_fg)),
                    ]))
                })
                .collect::<Vec<_>>()
        };
    ...
    fn render_change_agent_provider_prompt(&mut self, frame: &mut Frame) {
        let PromptState::ChangeAgentProvider(prompt) = &self.prompt else {
            return;
        };
        self.render_dim_overlay(frame);
    ...
    fn render_change_default_provider_prompt(&mut self, frame: &mut Frame) {
        let PromptState::ChangeDefaultProvider(prompt) = &self.prompt else {
            return;
        };
        self.render_dim_overlay(frame);
        let area = centered_rect(72, 42, frame.area());
    ...
    fn render_change_project_default_provider_prompt(&mut self, frame: &mut Frame) {
        let PromptState::ChangeProjectDefaultProvider(prompt) = &self.prompt else {
            return;
        };
        self.render_dim_overlay(frame);
    ...
    fn render_manage_worktrees_prompt(&mut self, frame: &mut Frame) {
        ...
        let rows =
            manage_worktree_visual_rows(&prompt.entries, prompt.loading, prompt.error.as_deref());
        let path_col = prompt
            .entries
            .iter()
            .map(|entry| {
        ...
        let items = rows
            .iter()
            .map(|row| match row {
                ManageWorktreeVisualRow::Header(label) => ListItem::new(Line::from(Span::styled(
                    format!(" {label}"),
                    Style::default()
                        .fg(self.theme.help_section_header_fg)
                        .add_modifier(Modifier::BOLD),
                ))),
                ManageWorktreeVisualRow::Empty(message) => ListItem::new(Line::from(Span::styled(
                    format!("  {message}"),
                    Style::default().fg(self.theme.hint_dim_desc_fg),
                ))),
    ...
    fn render_pick_project_worktree_prompt(&mut self, frame: &mut Frame) {
        let PromptState::PickProjectWorktree(prompt) = &self.prompt else {
            return;
        };
        self.render_dim_overlay(frame);
        let area = centered_rect(78, 58, frame.area());
        ...
        let rows =
            project_worktree_visual_rows(&prompt.entries, prompt.loading, prompt.error.as_deref());
        let path_col = prompt
            .entries
            .iter()
        ...
        let items = rows
            .iter()
            .map(|row| match row {
                ProjectWorktreeVisualRow::Header(label) => ListItem::new(Line::from(Span::styled(
                    format!(" {label}"),
                    Style::default()
                        .fg(self.theme.help_section_header_fg)
                        .add_modifier(Modifier::BOLD),
    ...
    fn render_confirm_kill_running_prompt(&mut self, frame: &mut Frame) {
        ...
        let (agent_count, terminal_count) = confirm_prompt.target_ids.iter().fold(
            ...
            |(agents, terminals), target_id| match target_id {
                RuntimeTargetId::Agent(_) | RuntimeTargetId::Tab(_) => (agents + 1, terminals),
    ...
    fn render_add_project_failed_prompt(&mut self, frame: &mut Frame) {
        let PromptState::AddProjectFailed {
            message, scroll, ..
        } = &self.prompt
        else {
            return;
        ...
        self.overlay_layout.active = OverlayMouseLayout::AddProjectFailed { ok_button: ok_area };
    }
    ...
    fn render_agent_info_prompt(&mut self, frame: &mut Frame) {
        let PromptState::AgentInfo(prompt) = &self.prompt else {
            return;
        };
        self.render_dim_overlay(frame);
    ...
    fn render_confirm_delete_terminal_prompt(&mut self, frame: &mut Frame) {
        let PromptState::ConfirmDeleteTerminal {
            terminal_label,
            foreground_cmd,
            focus,
            ..
    ...
    fn render_confirm_close_tab_prompt(&mut self, frame: &mut Frame) {
        ...
        let live_tabs = self
            ...
            .into_iter()
            .filter(|id| self.engine.providers.contains_key(id.as_ref_id()))
            .count();
        let will_detach = close_tab_detaches(
            self.engine.providers.contains_key(TabIdRef::new(tab_id)),
            live_tabs,
        );
        let agent_name = self
            .engine
    ...
    ///
    /// The body is `dux_core::engine::detach_confirm_body`, the same sentence
    /// the browser's dialog renders, so the two surfaces cannot promise
    /// different things about the same act.
    fn render_confirm_detach_agent_prompt(&mut self, frame: &mut Frame) {
    ...
    fn render_confirm_init_repo_prompt(&mut self, frame: &mut Frame) {
        let PromptState::ConfirmInitRepo {
            path,
            candidates,
            focus,
            ..
    ...
    fn render_debug_input_prompt(&mut self, frame: &mut Frame) {
        ...
        let visible: Vec<Line> = lines.iter().skip(start).take(visible_h).cloned().collect();
    ...
    fn render_pull_request_input_prompt(&mut self, frame: &mut Frame) {
        let PromptState::PullRequestInput {
            project,
            input,
            focus,
        } = &self.prompt
    ...
    fn render_attach_pull_request_input_prompt(&mut self, frame: &mut Frame) {
        let PromptState::AttachPullRequestInput {
            current_pr, input, ..
        } = &self.prompt
        else {
            return;
    ...
    fn render_configure_prompt(&mut self, frame: &mut Frame) {
        ...
        let is_env = matches!(
            &self.prompt,
            PromptState::ConfigureProjectEnv { .. } | PromptState::ConfigureGlobalEnv { .. }
        );
        ...
        render_input.set_display_width((text_area.width > 0).then_some(text_area.width as usize));
    ...
    fn render_startup_command_logs_prompt(&mut self, frame: &mut Frame) {
        ...
        let content_lines =
            crate::app::input::startup_command_log_visual_lines(&prompt.content, body_inner.width);
    ...
    fn render_kill_running_prompt(&mut self, frame: &mut Frame) {
        ...
        for (index, action) in buttons.iter().enumerate() {
            ...
            let press_target = match action {
                ...
                KillRunningFooterAction::Selected => ButtonPressedTarget::RuntimeKillSelected,
                KillRunningFooterAction::Visible => ButtonPressedTarget::RuntimeKillVisible,
            };
            let state = button_state_for(press_target, self.pressed_button, focused, enabled);
    ...
    fn kill_running_items(
        ...
        visible_indices
            ...
            .map(|runtime| {
                ...
                let mut spans = checkbox.inline_prefix(Style::default().fg(self.theme.hint_key_fg));
                spans.extend([
                    Span::styled(
    ...
    fn render_name_new_agent_prompt(&mut self, frame: &mut Frame) {
    ...
    fn render_prompt(&mut self, frame: &mut Frame) {
        if self.render_standalone_delete_prompt(frame) {
            return;
        }
        if self.render_picker_prompt_family(frame) {
            return;
        }
        if self.render_dialog_prompt_family(frame) {
            return;
        }
        self.render_editor_prompt_family(frame);
    }

    fn render_standalone_delete_prompt(&mut self, frame: &mut Frame) -> bool {
        let PromptState::ConfirmDeleteAgent {
            agent_label,
    ...
    fn render_picker_prompt_family(&mut self, frame: &mut Frame) -> bool {
        match &self.prompt {
    ...
    fn render_dialog_prompt_family(&mut self, frame: &mut Frame) -> bool {
        match &self.prompt {
    ...
    fn render_editor_prompt_family(&mut self, frame: &mut Frame) {
        match &self.prompt {
    ...
    fn center_pane_agent_title(&self) -> String {
        if let Some(session) = self.selected_session() {
            ...
            let remote = self
                .engine
                .providers
                .get(TabIdRef::new(&self.focused_tab_id(&session.id)))
...
/// The `ConfirmCloseTab` dialog's warning tail. Two independent consequences,
/// each stated only when it applies: whether the close takes the agent's last
/// live tab with it (which detaches the agent), and whether it hands the
...
mod tests {
    ...
    fn the_remote_count_sums_over_an_agents_tabs() {
        ...
        for tab_id in [slot_tab, "tab-2".to_string()] {
            ...
            app.engine.providers.insert(TabId::new(tab_id), client);
        }
    ...
    /// Closing a tab while siblings are live is destructive (the row is
    /// permanently deleted). The way back is a NEW tab, which always starts
    /// fresh, so the copy points at the provider's own history command rather
    /// than promising a resume.
    #[test]
    fn confirm_close_tab_tail_is_destructive_for_extra_tab_with_siblings() {
        let tail = confirm_close_tab_tail(false, None);
        ...
        assert!(!tail.contains("takes its place"), "tail: {tail}");
    }
    ...
    fn confirm_close_tab_tail_last_live_tab_detaches() {
        assert!(confirm_close_tab_tail(true, None).contains("last live tab"));
    }

    /// Closing the tab in the session slot hands the slot on, so the tail names
    /// the successor. That is a separate fact from detaching, and both are said
    /// when both apply (a dormant successor takes the slot AND the agent
    /// detaches, because the close took its last live process).
    #[test]
    fn confirm_close_tab_tail_names_the_tab_taking_the_slot() {
        let tail = confirm_close_tab_tail(false, Some("Codex"));
        assert!(
            tail.contains("The next tab, Codex, takes its place as the agent's first tab."),
            "tail: {tail}"
        );
        assert!(tail.contains("deletes this tab for good"), "tail: {tail}");

        let both = confirm_close_tab_tail(true, Some("Codex"));
        assert!(both.contains("last live tab"), "tail: {both}");
        assert!(both.contains("takes its place"), "tail: {both}");
    }

    /// The close-tab dialog's body does not scroll, and the promotion tail is
    /// the longest copy it can carry, so the modal must actually FIT the
    /// smallest terminal dux supports. Rendered rather than reasoned about,
    /// because that is exactly what the arithmetic gets wrong.
    #[test]
    fn the_close_tab_dialog_fits_an_eighty_by_twenty_four_terminal() {
        use ratatui::Terminal;
        use ratatui::backend::TestBackend;
        ...
        // The tail's last words, taken from the same pure copy the modal
        // renders, so a wording change cannot silently defeat the check.
        let tail = confirm_close_tab_tail(false, Some("Codex"));
        let last_words = tail.rsplit(' ').take(3).collect::<Vec<_>>();
        for word in last_words {
            assert!(
                screen.contains(word),
                "the promotion tail is clipped at 80x24; looked for {word:?} in:\n{screen}"
            );
        }
        assert!(
            screen.contains("Cancel") && screen.contains("Close"),
            "the buttons are clipped at 80x24:\n{screen}"
        );
    }
    ...
    fn tab_strip_ordinals_renumber_when_a_tab_closes() {
        ...
        app.engine.agent_tabs.remove(TabIdRef::new("tab-2"));
        let labels = render_labels(&mut app);
        assert!(
    ...
    fn activity_never_moves_a_narrow_strips_scroll_position() {
        ...
        for working in [false, true] {
            ...
            if working {
                ...
                app.engine.needs_attention.insert(TabId::new("tab-3"));
            }
    ...
    #[test]
    fn the_tailscale_picker_publishes_its_rows_and_current_mode_marker() {
        let mut app = test_app(default_bindings());
        app.open_set_tailscale_mode_prompt();
        let buf = render_to_buffer(&mut app);
        let (list, items) = match app.overlay_layout.active {
            OverlayMouseLayout::SetTailscaleMode { list, items, .. } => (list, items),
            ref other => panic!("expected tailscale picker layout, got {other:?}"),
        };
        assert_eq!(items, 3);
        let rows = (0..list.height)
            .map(|row| row_text(&buf, list, row))
            .collect::<Vec<_>>();
        assert!(rows.iter().any(|row| row.contains("auto")));
        assert!(
            rows.iter()
                .any(|row| row.contains(super::ACTIVE_PROVIDER_MARKER))
        );
    }
