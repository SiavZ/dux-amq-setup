...
// the bundled full-colour duck, a curated colour name gives the duck silhouette
// tinted with it as an inline SVG data URI.
//
// Only a curated colour name is accepted; a hex, a URL or a dropped name
// degrades to the default duck. The colour reaching the generated SVG is always
...
// read as negative space. Regenerating is manual (`npm i --no-save jimp potrace`
// then `node scripts/gen-duck-favicon.mjs`) and potrace's output is
// nondeterministic, so validate the duck visually before committing:
// `duckPath.test.ts` catches gross corruption only, never staleness.
const DUCK_PATH =
...
// `crates/dux-core/src/wire.rs`. No yellow: that is the unset default duck's own
 ...
 * uncurated yield the bundled duck.
 */
...
/**
 * Build an inline SVG `data:` URI of the duck silhouette filled in `color`, which
 ...
 * values render the default duck, and `applyFavicon` announces that once.
 */
...
// The attention dot and its dark rim, for contrast against the duck. The fill is
