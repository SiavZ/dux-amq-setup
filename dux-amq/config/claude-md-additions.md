...
- Do not choose transports yourself for normal routing. Do not call `amq` or Claude Peers directly unless the user explicitly asks for low-level diagnostics. Dux requires Claude Peers for Claude-to-Claude routes and uses AMQ for mixed Claude/Codex/Gemini routes.
